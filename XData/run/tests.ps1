...
// import { Console, bold, lightRed, yellow } from "https://deno.land/x/quickr@0.6.36/main/console.js"
import { Console, bold, lightRed, yellow } from "../../../quickr/main/console.js"
import { run, Timeout, Env, Cwd, Stdin, Stdout, Stderr, Out, Overwrite, AppendTo, throwIfFails, returnAsString, zipInto, mergeInto } from "https://deno.land/x/quickr@0.6.36/main/run.js"
...
for (let each of await glob(/main\/.+\.test\..+/)) {
    ...
    if (each.isFile) {
        ...
        try {
            var { success } = await run(`deno`, "run", "-Aq", each.basename, Out(Overwrite(logPath)), Env({NO_COLOR:"true"}))
        } catch (error) {
            console.log(bold.lightRed`    Error with test: `.yellow`${relativePart}:`.blue` ${error.message||error}`)
            continue
        }
        if (!success) {
